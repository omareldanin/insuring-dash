import type { Life } from "../../../services/rules";
import { deleteLife, upsertLifeRules } from "../../../services/rules";
...
export function LifeTable({ data }: Props) {
  const [ids, setIds] = useState<number[]>([]);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editRowId, setEditRowId] = useState<number | null>(null);
  const [editData, setEditData] = useState<Partial<Life>>({});
  ...
  return (
    <div className="mt-6">
      <div className="flex justify-between items-center">
        <h2 className="font-bold mb-3 text-gray-800">تأمين حياه</h2>
        <button
