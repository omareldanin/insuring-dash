...
export default function EditCardFeatures() {
  const navigate = useNavigate();

  const { data, isLoading } = usePlan(56);
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (payload: any) => updatePlan(56, payload),
    onSuccess: () => {
      toast.success("تم تعديل الباقة بنجاح");
      queryClient.invalidateQueries({ queryKey: ["plans"] });
      queryClient.invalidateQueries({ queryKey: ["plan", 56] });
      navigate("/cards");
    },
  ...
  return (
    <div className="bg-white shadow rounded-2xl p-6 max-w-3xl mx-auto">
      <h1 className="text-xl font-bold mb-4 text-[#121E2C]">
        تعديل مميزات كرت الخصم
      </h1>

      {/* ===== Name ===== */}
      <div className="mb-4">
        <label className="block mb-1 text-[#121E2C]">عنوان (AR)</label>
        <input
          value={nameAr}
      ...
      <div className="mb-4">
        <label className="block mb-1 text-[#121E2C]">عنوان (EN)</label>
        <input
          value={nameEn}
